...
local function add_highlight(buffer, ns, hl, line, from, to)
  ...
  -- })
  -- TODO: TWO
  io.write( "add_highlight called with:\n" )
  io.write( "    buffer = " .. tostring(buffer) .. "\n" )
  io.write( "    ns     = " .. tostring(ns) .. "\n" )
  io.write( "    hl     = " .. tostring(hl) .. "\n" )
  io.write( "    line   = " .. tostring(line) .. "\n" )
  io.write( "    from   = " .. tostring(from) .. "\n" )
  io.write( "    to     = " .. tostring(to) .. "\n" )
  io.flush()
...
function M.highlight(buf, first, last, _event)
  ...
  for l, line in ipairs(lines) do
	...
	io.write( vim.inspect(Config) .. "\n" )
	io.write( "kw = " .. tostring(kw) .. "\n" )
    io.flush()
    ...
    if opts then
      ...
      if not is_multiline then
        ...
        if opts.signs ~= nil then
		  ...
		  io.write( "show_sign = ".. vim.inspect( show_sign ) .. "\n" )
        end
        if show_sign then
          vim.fn.sign_place(
            0,
