...
function M.setup(options)
  ...
  M.log = io.open( pid .. "-todo-comments.log", "a" );
  io.output( M.log );
end
...
-- If we are using terminal gui colors, then translate the hex
-- values into X11 colors and use ctextfg/bg rather than guifg/bg.
--
-- TODO: Finish this!
--
-- June 27 2024 - Greg Rigole

function rgb_to_cterm_colour( rgb_colour )

	io.write( "rgb_to_cterm_colour() called with " .. tostring(rgb_color) )
...
function M.colors()
  ...
  for kw, opts in pairs(M.options.keywords) do
	...
	io.write( "kw = " .. vim.inspect(kw) .. "\n" )
	io.write( "kw_color = " .. vim.inspect(kw_color) .. "\n" )
    local hex
    ...
    if kw_color:sub(1, 1) == "#" then
      ...
      for _, color in pairs(colors) do
		io.write( "color = " .. vim.inspect(color) .. "\n" )
        if color:sub(1, 1) == "#" then
          hex = color
		  io.write( "color is hex\n" )
          break
        end
        local c = Util.get_hl(color)
		io.write( "c = " .. vim.inspect(c) .. "\n" )
	...
	io.write( "vim.opt.termguicolors._value = " .. vim.inspect( vim.opt.termguicolors._value ) .. "\n" )
	if vim.opt.termguicolors._value == true then
		io.write( "Using gui colors\n" )
